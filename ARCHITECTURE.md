...
| Subresource tampering (a served bundle script/style is swapped) | **SRI deliberately not adopted** (spike, follow-up to the CSP tightening above; see D11). SRI pins an asset by cryptographic hash — its payoff is for *cross-origin or mutable* resources. Mapped onto MEW that payoff is ~zero on both delivery models: **(a) the self-hosted bundle is same-origin and already Vite content-hashed** — the filename *is* the content digest, so a swapped asset is a new URL the HTML never references; an `integrity` attribute would only re-pin a same-origin asset already inside the app's own trust boundary, at the cost of a bespoke post-build inject step (Vite/Rollup emit no `integrity` natively) that the keyless build must never break. **(b) the only cross-origin script — Google Identity Services (`https://accounts.google.com/gsi/client`) — is un-pinnable**: Google rotates it and publishes no stable SRI hash, so a pinned `integrity` would hard-break sign-in on every rotation; it is also injected at *runtime* (`createElement('script')` in `adapters/calendar/google.ts`), not a static tag the HTML or CSP can carry SRI on. The honest control is the CSP itself: `script-src`/`style-src` admit only `'self'` + that one audited GIS origin, and `object-src 'none'` + `base-uri 'self'` close the rest. Guarded in `adapters/__tests__/csp.test.ts` (no foreign script/style origin creeps in; built resource tags stay same-origin). |
