...
- The bundle gate now measures everything MEW loads eagerly — the entry plus the chunks it pulls
  in at start — as one sum against one ceiling, and the gate's own policy tests run in CI, so a
  heavier start can't hide in a side chunk.
