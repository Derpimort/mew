...
A `#NNN` in this repo's older prose (CHANGELOG sections below `[2026.9.0]`, and some code comments)
is a `mew-archive` number from before the migration — **not** this repo's issue of that number, which
in most cases now exists and is about something else. The archive is private, so the link answers
404 unless you are a maintainer: that 404 is the expected answer, not a broken doc. Read such a
citation as provenance you cannot follow rather than as a pointer into this repo. (A bare `#N` in
`ARCHITECTURE.md` is an internal index — an acceptance criterion or a nudge — and not a citation at
all.)
