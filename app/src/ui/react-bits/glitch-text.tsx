...
const GlitchText: React.FC<GlitchTextProps> = ({
  ...
  const measureText = useCallback(
    (ctx: CanvasRenderingContext2D, str: string): number => ctx.measureText(str).width,
    []
  )
  ...
  const getAngle = useCallback(
    (x1: number, y1: number, x2: number, y2: number): number => Math.atan2(y2 - y1, x2 - x1),
    []
  )
  ...
  const initChars = useCallback(
    (
      ...
      if (autoFit) {
        effectiveFontSize = calculateFitFontSize(ctx, content, canvasWidth, canvasHeight)
      }
      ...
      for (let colorIndex = 0; colorIndex < allColors.length; colorIndex++) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          for (let j = 0; j < lines[i].length; j++) {
            ...
            const yPos = singleLine ? centerY : i === 0 ? centerY - lineH / 2 : centerY + lineH / 2
  ...
  const handleResize = useCallback(() => {
    ...
    charsRef.current = initChars(ctx, text, cx, cy, canvas.width, canvas.height)
