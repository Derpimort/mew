...
const Preloader: React.FC<PreloaderProps> = ({
      ...
      >
        {stairs.map((_, index) => (
          <motion.div
            ...
            exit={isReducedFade ? { opacity: 0 } : shouldAnimate ? { y: exitY } : { opacity: 0 }}
            transition={{
          ...
          >
            {!bgColor && <div className="w-full h-full bg-black dark:bg-[#5227FF]" />}
          </motion.div>
        ))}
        {renderLoadingText()}
      </div>
    ...
    return (
      <motion.div
        ...
        className={cn(
          ...
          bgColor ? '' : 'bg-oklch(1 0 0) dark:bg-oklch(0.145 0 0)'
        )}
        style={{
          zIndex,
      ...
      >
        <div
          className={cn(
            'text-[min(25vw,200px)] font-bold leading-none text-oklch(0.145 0 0) tabular-nums tracking-tighter dark:text-oklch(0.985 0 0)',
            percentageTextClassName
          )}
        >
          {displayProgress}
        ...
        {showProgressBar && (
          <div
            className={cn(
              'absolute left-0 right-0 h-0.5 bg-oklch(0.922 0 0) dark:bg-oklch(1 0 0 / 10%)',
          ...
          >
            <motion.div
              ...
              className="h-full bg-oklch(0.145 0 0) dark:bg-oklch(0.985 0 0)"
            />
          </div>
        )}
      </motion.div>
  ...
  return (
    <div className={cn('relative w-full h-full', className)}>
      ...
      <div className={cn('w-full h-full', showPreloader && 'invisible')}>{children}</div>
    </div>
  )
}
